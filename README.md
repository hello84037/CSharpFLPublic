...
## Running the sample
`SBFLApp` accepts the solution directory and the test project name. It instruments the discovered tests in place, executes them, aggregates the coverage data, and ranks statements by suspiciousness. Run the utility from the repository root:
...
To discard instrumentation and delete generated coverage artifacts, supply the optional `--reset` (or `-r`) switch:

```bash
dotnet run --project SBFLApp/SBFLApp.csproj . MathApp.Tests --reset
```

This cleans modified test sources, removes coverage files, and clears cached GUID mappings.
